# opalg/interpreter/code_generator.py

from opalg.compiler.ast_nodes import (
    ProgramNode, VariableDeclarationNode, FunctionDeclarationNode,
    ReturnStatementNode, ShowStatementNode, SaveStatementNode,
    DeleteStatementNode, IfStatementNode, WhileStatementNode, ForStatementNode,
    ExpressionStatementNode, BinaryOperationNode, UnaryOperationNode,
    NumberLiteralNode, StringLiteralNode, BoolLiteralNode, IdentifierNode,
    FunctionCallNode, MemberAccessNode, AwaitExpressionNode,
    VoidType, IntType, FloatType, StringType, BoolType, CustomType, TypeNode
)
...
class CodeGenerator:
    ...
    def generate_VariableDeclarationNode(self, node):
        ...
        const_kw = 'const ' if getattr(node, 'const', False) else ''
        value = self.generate_expression(node.value)
        self.output.append(f'{const_kw}{var_type} {node.name} = {value};')
    
    def generate_FunctionDeclarationNode(self, node):
        async_prefix = 'async ' if getattr(node, 'async_function', False) else ''
        return_type = self.get_type(node.return_type)
        params = ', '.join(
            f"{self.get_type(param.param_type)} {param.name}"
    ...
    def generate_ExpressionStatementNode(self, node):
        expr = self.generate_expression(node.expression)
    ...
    def get_type(self, type_node):
        if isinstance(type_node, VoidType):
            ...
            # If you have generics, handle them
            generic_args = [self.get_type(arg) for arg in type_node.generic_args]
            return f'{type_node.name}<{", ".join(generic_args)}>'
        else:
            return 'var'
